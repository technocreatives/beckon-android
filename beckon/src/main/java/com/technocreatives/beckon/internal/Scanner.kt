...
internal fun BluetoothLeScannerCompat.scan(setting: ScannerSetting): Observable<BeckonScanResult> {
    return Observable.create<BeckonScanResult> { observer ->
        ...
        this.startScan(setting.filters, setting.settings, callback)
...
internal fun BluetoothLeScannerCompat.scanList(setting: ScannerSetting): Observable<List<BeckonScanResult>> {
    // need to filter connected devices or saved devices
    return this.scan(setting)
